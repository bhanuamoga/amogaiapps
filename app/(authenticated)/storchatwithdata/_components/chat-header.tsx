...
export default function ThreadHeaderSticky({ chatUuid }: ThreadHeaderStickyProps) {
  ...
  useEffect(() => {
    async function loadChatTitle() {
      try {
        const res = await fetch(`/api/storchatwithdata/title?chatId=${chatUuid}`);
        ...
        if (data?.title) {
          ...
          await fetch("/api/storchatwithdata/title", {
            method: "PUT",
  ...
  async function saveChatTitle(newTitle: string) {
    try {
      ...
      const res = await fetch("/api/storchatwithdata/title", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  function handleNewChat() {
    const newId = uuid();
    router.push(`/storchatwithdata/${newId}`);
  }
      ...
      <div className="flex items-center justify-end gap-1 sm:gap-2 flex-shrink-0">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="z-[9999]">
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => router.push("/storchatwithdata/prompthistory")}>Prompt History</DropdownMenuItem>
             <DropdownMenuItem onClick={() => router.push("/storchatwithdata/History")}>
                Chat History
              </DropdownMenuItem>
            <DropdownMenuItem onClick={() => router.push("/storchatwithdata/woo-prompt-list")}>Store Prompt List</DropdownMenuItem>
            <DropdownMenuItem onClick={() => router.push("/storchatwithdata/prompt-schedule-logs")}>StorePrompt-schedule-logs</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
    </header>
