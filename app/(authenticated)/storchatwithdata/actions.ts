...
export async function loadChat(chatId: string) {
  ...
  const { data: chat, error: chatError } = await postgrest
    ...
    .eq("id", chatId)
    .eq("user_id", userId)
    .eq("chat_group", "StorChat With Data") // 🔥 ensure only this group
    .single();
...
const CHAT_GROUP = "StorChat With Data";
...
export async function getPromptHistory() {
  ...
  try {
    ...
    const { data: chats, error: chatError } = await postgrest
      ...
      .select("id, title")
      .eq("user_id", userId)
      .eq("chat_group", "StorChat With Data");
