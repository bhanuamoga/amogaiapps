...
export function MCPServerList({ isOpen, onClose }: MCPServerListProps) {
  ...
  const fetchServers = async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/woo-langchin-agent/mcp-servers");
      if (response.ok) {
  ...
  const deleteServer = async (id: string, serverName: string) => {
    ...
    setDeletingId(id);
    try {
      const response = await fetch(`/api/woo-langchin-agent/mcp-servers?id=${id}`, {
        method: "DELETE",
      });
      if (response.ok) {
          ...
          <div className="flex items-center justify-between border-b border-border p-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                disabled={loading}
                className="flex cursor-pointer items-center gap-1 px-2 py-1.5 text-sm text-muted-foreground transition-colors hover:text-foreground"
                title="Refresh"
              >
                {loading ? (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">
                            <span className="mr-2 inline-block rounded bg-muted px-2 py-0.5 text-xs">
                              {server.type}
                            </span>
                            {server.type === "stdio" ? (
                    ...
                    <div className="flex items-center gap-2">
                      <label className="relative inline-flex cursor-pointer items-center">
                        ...
                        />
                        <div className="peer peer-checked:bg-primary peer-focus:ring-ring/40 h-5 w-9 rounded-full bg-muted peer-focus:ring-4 peer-focus:outline-none after:absolute after:top-[2px] after:left-[2px] after:h-4 after:w-4 after:rounded-full after:border after:border-input after:bg-background after:transition-all after:content-[''] peer-checked:after:translate-x-full peer-checked:after:border-background"></div>
                      </label>
                      ...
                      <button
                        ...
                        className="cursor-pointer p-1.5 text-destructive transition-colors hover:text-destructive/80 disabled:cursor-not-allowed disabled:opacity-50"
                        title="Delete"
                      >
