...
export function ThreadList({ onOpenMCPConfig }: ThreadListProps) {
  ...
  return (
    <nav className="flex h-full flex-col border-r border-border bg-background/60 backdrop-blur-sm dark:border-border dark:bg-background/60">
      <div className="space-y-2 px-3 pt-3 pb-2">
        ...
        <div className="group relative">
          ...
          <input
            ...
            className="w-full rounded-md border border-input bg-background/40 py-1.5 pr-2 pl-8 text-xs focus:ring-2 focus:ring-ring/40 focus:outline-none"
          />
        </div>
      </div>
      <div className="flex-1 space-y-1 overflow-y-auto px-2 pb-3">
        {filtered.map((thread) => {
          const active = pathname === `/store-ai-chat/${thread.id}`;
          ...
          return (
            <div
              ...
              onClick={() => {
                if (!isRenaming) router.push(`/store-ai-chat/${thread.id}`);
              }}
            >
              {!isRenaming && (
                <div className="space-y-2">
                  ...
                  {thread.tokenUsage && (
                    <TokenUsageDisplay tokenUsage={thread.tokenUsage} compact />
                  )}
                </div>
              )}
              {isRenaming && (
        ...
        {filtered.length === 0 && (
          <div className="px-3 py-6 text-center text-xs text-muted-foreground">No threads found.</div>
        )}
      </div>

      {/* MCP Configuration Button */}
      <div className="border-t border-border p-3 hidden">
        <button
          onClick={onOpenMCPConfig}
          className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-md bg-muted px-3 py-2 text-xs font-medium text-muted-foreground transition-colors hover:bg-muted/80"
        >
